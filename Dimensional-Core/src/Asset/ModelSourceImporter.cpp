...
static Ref<SubMesh> processMesh(aiMesh* mesh, const aiScene* scene, AssetHandle& handle, const std::filesystem::path& modelPath)
{
    ...
    return SubMesh::Create(data);
}

static void processNode(aiNode* node, const aiScene* scene, std::vector<Ref<SubMesh>>& meshes, const std::filesystem::path& modelPath)
{
...
Ref<SubMesh> ModelSourceImporter::loadModelSourceFromPath(std::filesystem::path path)
{
    ...
    std::vector<Ref<SubMesh>> ms;
    processNode(scene->mRootNode, scene, ms, path);
    //
